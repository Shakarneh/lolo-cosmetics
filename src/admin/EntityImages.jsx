...
export function makeImageUrl(bucket) {
  return (storagePath) => supabase.storage.from(bucket).getPublicUrl(storagePath).data.publicUrl
}

// Generic multi-image manager (crop-before-upload, main-image picker, zoom/delete) used for
// both product photos and package photos. `table`/`bucket`/`fkColumn` point it at the right
// place; the upload/crop/position logic itself is identical for either entity.
function EntityImages({ entityId, table, bucket, fkColumn, title = 'الصور', onCountChange }) {
  ...
  const imageUrl = makeImageUrl(bucket)
  ...
  }, [table, fkColumn, entityId, onCountChange])
  ...
  function handleFiles(e) {
    ...
    if (images.length + files.length > MAX_IMAGES) {
      setError(`الحد الأقصى ${MAX_IMAGES} صورة`)
      return
    }
    setError(null)
  ...
  async function uploadBlob(blob) {
    const path = `${entityId}/${Date.now()}-${Math.random().toString(36).slice(2, 8)}.webp`
    const { error: upError } = await supabase.storage.from(bucket).upload(path, blob, { contentType: 'image/webp' })
    if (upError) throw upError
    const { error: insError } = await supabase
      .from(table)
      .insert({ [fkColumn]: entityId, storage_path: path, position: posRef.current++ })
    if (insError) {
  ...
  async function handleDelete(img) {
    ...
    setBusy(true)
    setError(null)
    const { error: delError } = await supabase.from(table).delete().eq('id', img.id)
    if (delError) {
  ...
  async function makeMain(img) {
    ...
    const { error: updError } = await supabase.from(table).update({ position: mainPosition - 1 }).eq('id', img.id)
  ...
  return (
    <div className="rounded-2xl bg-white border border-rose/15 p-6 flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <h2 className="font-bold text-rose-dark">{title}</h2>
